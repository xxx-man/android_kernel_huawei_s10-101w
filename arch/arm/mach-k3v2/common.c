...
#if 0 /*S10_HWID_L3H(S10, S10101, A)*/
#include <mach/board_s10_hi6421_regulator.h>
#else
...
static int __init early_get_memsize(char *p)
{
	...
	renew_mem_array(memsize);
	return 0;
}
early_param("mem_size", early_get_memsize);

unsigned long s10_get_mem_size(void)
{
	return memsize;
}

EXPORT_SYMBOL(s10_get_mem_size);
...
static void k3v2_mem_setup(void)
{
	...
	k3v2_overlay_compose_memory_setup();
#endif
	/*
...
static int __init early_k3v2_mem(char *p)
{
	...
	while (*p != '\0') {
		...
		// HHHH only for 2G
		//size = SZ_128M;
		//size = SZ_32M +SZ_64M + SZ_32M +SZ_64M;
		//size = SZ_256M + SZ_32M + SZ_8M;
		//size = SZ_1M;
...
static int /* void */ dev_name##_reset(void){						\
	int ret;							\
									\
...
static struct platform_device device_gpio_i2c_adpt = {
	.name = GPIO_I2C_ADTP_NAME,
	.id	= 4,
	.dev = {
		.platform_data = &gpio_i2c_adpt_pdata,
	},
};
#endif
...
/* bus_clk, div, drv_sel, sam_sel_max, sam_sel_min, input_clk */
	...
	{
		360000000, 6, 4, 3, 0, 50000000		/* 2: SDR25 */
	},
	{
		720000000, 6, 1, 11, 4, 100000000		/* 3: SDR50 */
	},
	{
		-1, -1, -1, -1, -1, -1					/* 4: SDR104 */
	},
	{
		360000000, 7, 1, 22, 15, 50000000		/* 5: DDR50 */
	},
};
...
/* device to manage power of balong flashless modem */ 
static struct balong_power_plat_data  balong_flashless_plat_data = {
	.flashless = true,
		.gpios = {
		[BALONG_RESET] 	   = {95, GPIOF_IN,  "balong_rst"},
			[BALONG_POWER_ON] 	   = {96, GPIOF_OUT_INIT_LOW, "balong_pow"},
...
static struct platform_device balong_flashless_power_device = {
	.name		= "balong_power",
		.id = -1,
		.dev = {
		.platform_data = &balong_flashless_plat_data,
    },
};
#endif
