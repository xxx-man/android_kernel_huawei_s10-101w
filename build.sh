...
if [ -e ${KERNELDIR}/arch/arm/boot/zImage ]; then
	...
	${KERNELDIR}/mkbootimg --kernel zImage --ramdisk initrd.cpio.gz --base 0x00008000 --pagesize 2048 --ramdiskaddr 0x01000000 --cmdline 'console=ttyS0 k3v2_pmem=1 vmalloc=512M androidboot.hardware=hws10101w androidboot.carrier=wifi-only mmcparts=mmcblk0:p1(xloader),p3(nvme),p4(misc),p5(splash),p6(oeminfo),p7(logo),p8(vrcb),p9(recovery2),p10(recovery),p11(boot),p12(modemimage),p13(modemnv),p14(modemnvm2),p15(cache),p16(system),p17(cust),p18(userdata),p19(reserve1),p20(reserve2),p21(reserve3),p22(data);mmcblk1:p1(sdcard2)' -o ${KERNELDIR}/boot.img
	...
	cd ${KERNELDIR}/#OUT/
	zip -r Kernel-`date +"[%d-%m-%y]-[%H-%M]-KK"`.zip ".config" "boot.img" "zImage"
	rm -f ${KERNELDIR}/initrd.cpio.gz ${KERNELDIR}/boot.img ${KERNELDIR}/zImage
	rm -f ${KERNELDIR}/#OUT/.config ${KERNELDIR}/#OUT/boot.img ${KERNELDIR}/#OUT/zImage
echo ""
echo "==================================================================="
