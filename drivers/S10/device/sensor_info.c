...
static ssize_t sensor_show_po188_input(struct device *dev,
                struct device_attribute *attr, char *buf)
{
    if (dev == NULL) {
        pr_err("sensor_show_po188_input dev is null\n");
        return -EINVAL;
    }
    return sprintf(buf, "%s\n", sensor_binder_input[PO188]);
}
static DEVICE_ATTR(lightsensor, S_IRUGO,
                   sensor_show_po188_input, NULL);
...
MODULE_AUTHOR("S10 driver group of k3v2");
MODULE_LICENSE("GPL");
