				...
				int poll_interval)
{
	...
	/* Add by skf57909 begin 2011/11/29 */
	if (i < 0)
		i = 0;
	/* Add by skf57909 end 2011/11/29 */
			       ...
			       const char *buf, size_t size)
{
	...
	if (ENABLE_SELFTEST == val) {
		...
		/*add w00185212  read gyroscope value*/
		mutex_lock(&gyro->lock);
...
static int sicoordinate_gyro = -1;
static ssize_t coordinate_gyro_show(struct device *dev, struct device_attribute *attr,
               char *buf)
{
    return sprintf(buf, "coordinate_gyro = %d\n",sicoordinate_gyro);
}

static ssize_t coordinate_gyro_store(struct device *dev, struct device_attribute *attr,
                const char *buf, size_t size)
{
    ...
    if ((status != 0) && (status != 1)) {
    	...
    	sicoordinate_gyro = status;
        if (sicoordinate_gyro == 1) {
            gyro->pdata->axis_map_x = 0;
            gyro->pdata->axis_map_y = 1;
            gyro->pdata->negate_y = 0;
        } else if (sicoordinate_gyro == 0) {
...
#endif
    __ATTR(coordinate_gyro, 0644, coordinate_gyro_show, coordinate_gyro_store),
};
...
static void l3g4200d_input_work_func(struct work_struct *work)
{
	...
	gyro = container_of((struct delayed_work *)work,
							struct l3g4200d_data, input_work);
	dev_dbg(&gyro->client->dev, "%s: l3g4200d_input_work_func start.\n", L3G4200D_GYR_DEV_NAME);
					...
					const struct i2c_device_id *devid)
{
	...
	/* modified by skf57909 begin 2011/12/14 */
	chipid = WHO_AM_I;
	...
	/* modified by skf57909 end 2011/12/14 */
	...
	if (err < 0) {
		...
		/* modified by skf57909 begin 2011/11/29 */
		goto err3;
		/* modified by skf57909 end 2011/11/29 */
	}
