...
char ddr_physize[DDR_PHYSIZE_LEN + 1] ;
...
static const char* major_version[] =
{
    ...
    "TBD", "TBD", "TBD", "TBD", "TBD", "STEN602uAPV0001", "STEN601uAPV0001", "STEN601wAPV0001",
    "TBD", "TBD", "TBD", "TBD", "TBD", "TBD", "SD2MMC392WK", "TBD",
    "TBD", "STEN201waAPV0002", "STEN201waAPV0001", "STEN201wdAPV0010", "TBD", "TBD", "STEN331uAPV0001", "STEN101wfAPV0001",
    "TBD", "TBD", "STEN201LAPV0002", "STEN201LAPV0001", "TBD", "STEN103LAPV0001", "STEN102LAPV0001", "STEN101LAPV0001",
    "TBD", "TBD", "STEN201wAPV0002", "STEN201wAPV0001", "STEN101wtAPV0004", "STEN101wAPV0003", "STEN101wAPV0002", "STEN101wAPV0001",
    "TBD", "TBD", "STEN202uAPV0002", "STEN202uAPV0001", "STEN202uAPV0003", "STEN102uAPV0003", "STEN102uAPV0002", "STEN102uAPV0001",
    "TBD", "TBD", "STEN201uAPV0002", "STEN201uAPV0001", "STEN201uAPV0003", "STEN101uAPV0003", "STEN101uAPV0002", "STEN101uAPV0001",
};
...
extern unsigned long s10_get_mem_size(void);
static int ddrphy_size_get(void)
{
       unsigned long size = s10_get_mem_size();
...
static ssize_t ddr_physize_attr_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{

    int ddr_pyhsize = 0;
    ddr_pyhsize = ddrphy_size_get();
    return sprintf(buf, "%d\n", ddr_pyhsize);
}
static struct kobj_attribute hw_version_attribute =
     __ATTR(hw_version, 0444, hw_version_attr_show, NULL);
static struct kobj_attribute ddr_physize_attribute =
     __ATTR(ddr_physize, 0444, ddr_physize_attr_show, NULL);


/*	
	func	: This function is used to determine the modem normalization. detect modem  LTE/WCDMA/TD/CDMA2000...
              The call in "product_feature_sel.c" is early than this function running.
			  so, this function needs its own determine hardware version
	input	: global variable  : hw_version
	output	: 	   'L' == LTE
			       'U' == WCDMA
			       'T' == TD-CDMA
			       'C' == CDMA2000
			       'X' == unknown 
*/
char modem_standard_detect(void)
{
	// S10 FHD && LINK
	if (!strncmp(hw_version,"STEN",4)){
		if( (hw_version[7] == 'L' ) || (hw_version[7] == 'l')){
			printk("modem_standard_detect  LTE :%c",hw_version[7]);
			return 'L';
		}
		else if( (hw_version[7] == 'U') || (hw_version[7] == 'u')){
			printk("modem_standard_detect  UUU:%c",hw_version[7]);
			return 'U';
		}
		else if( (hw_version[7] == 'W') || (hw_version[7] == 'w')){
			printk("modem_standard_detect   WWW:%c",hw_version[7]);
			return 'W';
		}
	}
	// MMC392
	else if(!strcmp(hw_version,"SD2MMC392WK")){
		printk("modem_standard_detect   MMC392");
		return 'U';
	}
	// other
	else{
		printk("modem_standard_detect : unknown");
		return 'X';
	}
	return 'X';
}
EXPORT_SYMBOL( modem_standard_detect );

static ssize_t modem_standard_attr_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%c\n", modem_standard_detect());
}
static struct kobj_attribute modem_version_attribute =
     __ATTR(modem_standard, 0444, modem_standard_attr_show, NULL);

static struct attribute* version_attributes[] =
{
     &hw_version_attribute.attr,
     &ddr_physize_attribute.attr,
     &modem_version_attribute.attr,
     NULL
};
