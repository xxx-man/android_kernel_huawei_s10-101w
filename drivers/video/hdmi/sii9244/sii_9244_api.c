...
#endif
int gMHLprintTag = 1; /* 1 for print and 0 for not print*/
static int gCloseMHLPower5V = 1;

int g_DEVCAP_VAL_BANDWIDTH = 0xF;
int g_MHL_LOGICAL_DEVICE_MAP = (MHL_DEV_LD_VIDEO | MHL_DEV_LD_GUI);
...
int vbusPowerState = POWER_STATE_NO_MHL;
...
void AppVbusControl(int powerOn)
{
#if defined(CONFIG_SUPPORT_MICRO_USB_PORT) 
    if (gCloseMHLPower5V)
    {
    ...
    TX_API_PRINT(("[MHL]AppVbusControl: powerOn State is %d!\n", powerOn));
    if (POWER_STATE_NO_MHL == powerOn)
    {/*MHL配件被拔出，需要通知charegr*/
        MHLSinkOrDonglePowerStatusCheck();
        TX_API_PRINT(("Mhl:API: Peer's POW bit is set. Turn the VBUS power OFF here.\n"));
        disable_vbus_irq();
        close_usb_out5v();
        notify_charger_type(CHARGER_TYPE_MHL_REMOVED);
        msleep(100);
        enable_vbus_irq();
        TX_API_PRINT(("[MHL]AppVbusControl: MHL Dongle is removed, notify the charger and close usb out5v!\n"));
    }
    else if (POWER_STATE_MHL_NO_POWER == powerOn)
    {/*MHL配件处于未连接外接电源的状态，需要将PAD电源打开*/
        TX_API_PRINT(("Mhl:API: Peer's POW bit is cleared. Turn the VBUS power ON here.\n"));
        disable_vbus_irq();
        open_usb_out5v();
        notify_charger_type(CHARGER_TYPE_MHL_NO_POWER);
        msleep(100);
        enable_vbus_irq();
        TX_API_PRINT(("[MHL]AppVbusControl: MHL Dongle is no power, notify the charger and open usb out5v!\n"));
    }
    else if (POWER_STATE_MHL_WITH_POWER == powerOn)
    {/*MHL配件处已连接外接电源的状态，需要将PAD电源关闭*/
        MHLSinkOrDonglePowerStatusCheck();
        TX_API_PRINT(("Mhl:API: Peer's POW bit is set. Turn the VBUS power OFF here.\n"));        
        disable_vbus_irq();
        close_usb_out5v();
        notify_charger_type(CHARGER_TYPE_MHL_POWER);
        msleep(100);
        enable_vbus_irq();
        TX_API_PRINT(("[MHL]AppVbusControl: MHL Dongle is powered, notify the charger and close usb out5v!\n"));
    }
    else
    {
        /*错误状态*/
        TX_API_PRINT(("[MHL]AppVbusControl: Power State ERROR!\n"));
    }
#endif    
}
#endif
...
#endif
static ssize_t mhl_sii9244_log_show(struct kobject *kobj, 
                                    struct kobj_attribute *attr, 
                                    char *buf)
{
    return sprintf(buf, "%d\n", gMHLprintTag);
}
extern  void SiiSwitchDisConnect(void);
extern  void SwitchToD0(void);
extern  void SwitchToD3(void);
extern  void ForceUsbIdSwitchOpen(void);
extern  void ReleaseUsbIdSwitchOpen(void);
extern  void processRgndOtg(void);
extern  void ForceSwitchToD3(void);
extern  void hdcp_set_enable(bool enable);
static ssize_t mhl_sii9244_log_store(struct kobject *kobj, 
                                     struct kobj_attribute *attr, 
                                     const char *buf, 
                                     size_t count)
{
    ...
    if (NULL != buf)
    {
        ret = sscanf(buf, "%d", &gMHLprintTag);
    }

    switch (gMHLprintTag)
    {
        case 2:
            (void)Sii9244_mhl_reset();
            break;
        case 3:
            SiiSwitchDisConnect();
            break;
    ...
    if (-1 == ret)
    {
        printk(KERN_ERR "sscanf gMHLprintTag error\n");
        return ret;
    }
    return count;
}

static ssize_t mhl_sii9244_close_power_show(struct kobject *kobj, 
                                    struct kobj_attribute *attr, 
                                    char *buf)
{
    return sprintf(buf, "%d\n", gCloseMHLPower5V);
...
static ssize_t mhl_sii9244_close_power_store(struct kobject *kobj, 
                                     struct kobj_attribute *attr, 
                                     const char *buf, 
                                     size_t count)
{
    ...
    if (NULL != buf)
    {
        ret = sscanf(buf, "%d", &gCloseMHLPower5V);
    }

    if (-1 == ret)
    {
        printk(KERN_ERR "sscanf gCloseMHLPower5V error\n");
        return ret;
    }
    return count;
}
...
static struct kobj_attribute mhl_sii9244_log_attribute = 
    __ATTR(log_en, 0664, mhl_sii9244_log_show, mhl_sii9244_log_store);

static struct kobj_attribute mhl_sii9244_close_power_attribute = 
    __ATTR(close_power, 0664, mhl_sii9244_close_power_show, mhl_sii9244_close_power_store);

static struct attribute *mhl_sii9244_attributes[] = 
{
    &mhl_sii9244_log_attribute.attr,
    &mhl_sii9244_close_power_attribute.attr,
    NULL
};
static struct attribute_group mhl_sii9244_attr_group = 
{
    .attrs = mhl_sii9244_attributes,
};
			...
			const struct i2c_device_id *dev_id)
{
	...
	if (mhl_Sii9244_page0 != NULL
        ...
        if (get_board_type() != E_BOARD_TYPE_LINK_S10)
        {   /*S10 FHD config*/
            g_DEVCAP_VAL_BANDWIDTH = 0;
            g_MHL_LOGICAL_DEVICE_MAP = (MHL_DEV_LD_AUDIO | MHL_DEV_LD_VIDEO | MHL_DEV_LD_MEDIA | MHL_DEV_LD_GUI);
        }
        ...
        kobj = kobject_create_and_add("mhl_sii9244", NULL);
        if (kobj == NULL)
        {
            TX_API_PRINT((KERN_ERR "MHL: kobj create failed!\n"));
            goto mhl_failed_9;
        }
        if (sysfs_create_group(kobj, &mhl_sii9244_attr_group))
        {
            TX_API_PRINT((KERN_ERR "MHL: sysfile create failed!\n"));
            goto mhl_failed_10;
        }
        gMHLprintTag = 0;
	}
...
mhl_failed_10:
    kobject_put(kobj);
mhl_failed_9:
    input_unregister_device(Sii9244->input);
mhl_failed_8:
#ifdef SiI9244DRIVER_INTERRUPT_MODE
...
static int mhl_Sii9244_remove(struct i2c_client *client)
{
    ...
    if ((Sii9244 != NULL) && (Sii9244->input != NULL))
    {
        input_unregister_device(Sii9244->input);
    }
	sysfs_remove_group(kobj, &mhl_sii9244_attr_group);
    if (kobj != NULL)
    {
        kobject_put(kobj);
    }
...
{
    if (sii9244work && mhl_Sii9244_page0 == cl)
    {
...
{
    if (mhl_Sii9244_page0 && mhl_Sii9244_page0 == cl)
    {
        enable_irq(mhl_Sii9244_page0->irq);
    }
	return 0;
