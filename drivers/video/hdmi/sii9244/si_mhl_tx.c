	...
	if (MHL_READ_DEVCAP == mhlTxConfig.mscLastCommand) {
		if (mhlTxConfig.mscLastOffset < sizeof(mhlTxConfig.aucDevCapCache)) {
			...
			if (MHL_DEV_CATEGORY_OFFSET == mhlTxConfig.mscLastOffset) {
                ...
                if (((data1 & MHL_DEV_CATEGORY_POW_BIT) == MHL_DEV_CATEGORY_POW_BIT) 
                    && (vbusPowerState != POWER_STATE_MHL_WITH_POWER)) {
					AppVbusControl(vbusPowerState = POWER_STATE_MHL_WITH_POWER);
				}
                else if (((data1 & MHL_DEV_CATEGORY_POW_BIT) != MHL_DEV_CATEGORY_POW_BIT) 
                    && (vbusPowerState != POWER_STATE_MHL_NO_POWER))
                {
