...
static unsigned int get_codec_mem_size(void)
{
	unsigned long size = s10_get_mem_size();

	if (size > SZ_1G) {
		return HISI_MEM_CODEC_SIZE_2G;
	} 

	return HISI_MEM_CODEC_SIZE;
}
			   ...
			   unsigned int cmd, unsigned long arg)
{
	...
	case MEMALLOC_IOCHARDRESET:{
			...
			destroy_list();
            initial_list(get_codec_mem_size(), g_baseaddr);	/*init head */
...
int __init memalloc_init(void)
{
	...
	if (INIT_FAILED == initial_list(get_codec_mem_size(), g_baseaddr)) {
